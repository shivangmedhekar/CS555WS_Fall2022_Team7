...
class Test_US09(unittest.TestCase):
    def test_child_birth_before_parents_death(self):
        ...
        try :
            ...
            write_errors(user_story = USER_STORY, error = e, no_fail = 1, total_fail = 2)
        ...
        try :
            ...
            write_errors(user_story = USER_STORY, error = e, no_fail = 2, total_fail = 2)
...
class Test_US20(unittest.TestCase):
    def test_aunts_and_uncles(self):
        ...
        for fam_id in families:
            try:
                self.assertTrue(us20.aunts_and_uncles(fam_id = fam_id, individuals=individuals, famalies=families))   
...
def write_errors(user_story, error, id='', no_fail = 1, total_fail = 1):
    
    error_msg = create_error_msg(user_story, error, id, no_fail, total_fail)
    print(error_msg)
...
def create_error_msg(user_story, error, id, no_fail, total_fail) -> str:
    if len(id):
        return f'[Fail {no_fail}/{total_fail}]ERROR: {user_story}: {id}: {error}'   
    else:
        return f'[Fail {no_fail}/{total_fail}]ERROR: {user_story}: {error}'      
